...
4. When all the pods are up and running, we can login to the UI of ArgoCD. Get the route and open it in a new browser tab.
    ...
    <p class="tip">
    ⛷️ <b>NOTE</b> ⛷️ - You can expect to receive an error from your browser stating that "Your connection is not private". This is because of the permissive mode we are allowing for ArgoCD and the fact we're using a .dev domain which enforces ssl which ArgoCD doesn't support.  To get around this error on Chrome you can simply type "thisisunsafe" directly in the browser.  This will feel odd because you are not entering it in a dialog box, but rather on the page itself.  In older versions of Chrome you simply selected 'advanced' and 'proceed'. 
    </p>
...
8. After you hit create, you'll see `our-todolist` application is created and should start deploying in your `${TEAM_NAME}-ci-cd` namespace.  It may take a few minutes for your the deployment to complete and your status changes to 'Healthy' and 'Synced'.
